...
area_stack_pre <- function(p) {
  ...
  # y_low <- rep(0, length(p$x))
  # for (j in 1:ncol(p$y)) if (is_area_stack(p$type[j])) {
  #   y_high                  <- y_low + p$y[, j]
  #   y_stack_list[[1 + length(y_stack_list)]] <- list(y_low = y_low, y_high = y_high)
  #   y_low                   <- y_high
  #   y_stack_list_index[j] <- length(y_stack_list)
  #
  #   # Adapt y_lim
  #   # range_to_add <- c(as.numeric(y_low), as.numeric(y_high))
  #   # if (p$y_force_include_zero) range_to_add <- range(c(range_to_add, 0), na.rm = T)
  #   # if (!p$y_lim_by_user & is_yl(p, j)) p <- update_y_lims(p, j, range_to_add)
  #   # if (is_yr(p, j))                    p <- update_y_lims(p, j, range_to_add) # TODO NOT if user sets yr
  # } else {
  #   y_stack_list_index[j] <- NA
  # }
...
area <- function(p) {
  ...
  for (index_obs_fc in seq_along(p$index_lst)) {
    ...
    for (j in 1:length(p$y_area_list)) {
      ...
      if ("forecast" == names(p$index_lst)[index_obs_fc]) {
        stop("po")
        polygon(x = x, y = y, col = get_col_from_p(p, p$forecast_shading_col), border = NA, density = cm(1) * p$forecast_shading_density, angle = p$forecast_shading_angle)
      }
    }
  }
...
area_stack <- function(p) {
  ...
  for (index_obs_fc in seq_along(p$index_lst)) {
    ...
    for (j in 1:ncol(p$y)) if (is_set(p$y_stack_list_index[j])) {
      ...
      this_col <- make_transparent(p$color[j], if (1 == index_obs_fc) 1 else 1 - p$forecast_col_transparency)
