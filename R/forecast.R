...
forecast_bg <- function(p) {
  ...
  if (p$turn) {
    rect(yl, xr, yh, xl, col = p$forecast_bg_col, border = NA)
  } else {
    rect(xl, yl, xr, yh, col = p$forecast_bg_col, border = NA)
  }
...
forecast <- function(p) {
  ...
  str_dim <- strwidth(p$forecast_text, cex = p$forecast_font_size, font = hack_font(p, p$forecast_font_style), units = "user", family = p$font)
  ...
  text(x = x, y = y, p$forecast_text, xpd = TRUE, adj= c(0.5, p$forecast_below_gridline), cex = p$forecast_font_size, font = hack_font(p, p$forecast_font_style), col = get_col_from_p(p, p$forecast_text_col), srt = if (p$turn) 270 else 0, family = p$font)
