...
bars <- function(p) {
  ...
  if (p$bar_lab_show) {
    ...
    todo(p, "Oók marge bij horizontale labels (in bars)")
  ...
  for (j in 1:ncol(p$y)) if (is_bar(p$type[j])) for (i in seq_along(p$x)) {
    ...
    #
    rect(xl, yl, xr, yh, col = this_col, border = NA)
    if (in_forecast & p$forecast_shading_show) {
      ...
      rect(xl, yl, xr, yh, col = this_col, border = NA, density = cm(1) * p$forecast_shading_density, angle = p$forecast_shading_angle)
    }
    ...
    if (p$bar_lab_show) {
      ...
      if (show_this_lab) {
        ...
        if (!p$turn) {
          ...
          text((xl + xr) / 2, this_y, bar_lab, srt = p$bar_lab_rotation, adj = align, cex = this_cex, font = p$bar_lab_font_style, col = p$bar_lab_col)
        } else {
          if (p$bar_lab_top) {
            if (is_bar_next(p$type[j]) | is_bar_stack_last(F) | is_bar_stack_last(T)) {
              ...
              this_cex <- if (is_set(p$bar_lab_font_size)) p$bar_lab_font_size else p$bar_lab_font_scaling
              text(this_x, (yl + yh) / 2, bar_lab, adj = align, cex = this_cex, font = p$bar_lab_font_style, col = p$bar_lab_col)
            }
