...
world_map_plot <- function(p) {
  ...
  for (i in 1:6) {
    xl <- x_left + (i - 1) * dx / 6
    xr <- xl + dx / 6
    rect(xl, y_bot, xr, y_top, col = p$color[i], border = NA)#, lwd = 1)
    
    # sep dots
    if (1 < i) points(xl, y_top, col = get_col_from_p(p, p$world_map_legend_dot_col), cex = p$world_map_legend_dot_size, pch = 19)
  
    text(x = xl, y = y_top + dy / 2, labels = txt[i], family = p$font, cex = p$world_map_legend_font_size, col = get_col_from_p(p, p$world_map_legend_font_col))
  }
