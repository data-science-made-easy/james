...
whisker_pre <- function(p) {
  ...
  for (i in 1:(length(index_whisker) / 2)) { # So i = 1, 3, 5, ..., n - 1
    ...
    if (is_yr(p, ref_series)) p$y_right_lim <- range(c(p$y_right_lim, low, high), na.rm = T)
    # if (is_set(p$y_right_lim)) error_msg("To do: implement this!")
  }
...
whisker <- function(p) {
  ...
  for (j in seq_along(p$whisker_list)) {
    ...
    if (any(is_bar(p$type))) {
      if (p$turn) {
        whisker_edge_length_inch <- min(whisker_edge_length_inch, p$bar_width / diff(par("usr")[c(4,3)]) * par("pin")[2])
      } else {
        whisker_edge_length_inch <- min(whisker_edge_length_inch, p$bar_width / diff(par("usr")[1:2]) * par("pin")[1])
      }
    }
    ...
    suppressWarnings(arrows(x0 = x0, y0 = y0, x1 = x1, y1, code = 3, col = this_col, angle = 90, length = whisker_edge_length_inch / cm(1), lwd = p$whisker_lwd))
  }
