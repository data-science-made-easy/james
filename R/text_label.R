text_label <- function(p) {
  ...
  if (!is_set(p$text_rotation)) {
    text(this_x, this_y, set_newline(p$text_label), family = p$font, pos = pos, col = get_col_from_p(p, p$text_col), srt = p$text_rotation, font = p$text_font_style, cex = p$text_font_size, offset = p$text_offset)
  } else {
    for (i in 1:length(p$text_label)) {
      ...
      text(this_x[i], this_y[i], set_newline(p$text_label[i]), family = p$font, pos = this_pos, col = this_col, srt = this_srt, font = this_stl, cex = this_cex, offset = this_off)
    }
  }
