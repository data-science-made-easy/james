james <- function(...) { # Arg may be list or individual values, i.e. james(list(a=1,b=2)) or james(a=1,b=2) or james()
  ...
  if (!is.element(GLOBALS, names(p)))   p[[GLOBALS]]   <- globals
...
get_parsed <- function(lst, name, allow_non_existing = FALSE) {
  ...
  # get globals from this object
  globals <- lst[[GLOBALS]]
  
  #
  ## Set style as user wants, with default as last (lowest priority)
  #
  style_vec <- globals[which(STYLE == globals$param), ]
  ...
  style <- unique(c(style, DEFAULT)) # make sure we always have default as fallback
  ...
  if (!length(index)) {
    ...
    return(val) # Not found in globals, so casting not possible
  } else {
    type <- globals[index, TYPE]
    for (this_style in style) if (!all(is_set(val))) val <- globals[index, this_style] # Set value if not set yet
  }
