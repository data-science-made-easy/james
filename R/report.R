...
fix_path <- function(path = '', use_local_path = F) { # Don't replace F with on_mac()
  path_prefix <- ""
  if (get_param("install_status_production")) { # PROD
    if (get_param("install_local")) { # PROD & LOCAL
...
install_root <- function() if (get_param("install_local")) get_param("install_root_local_prod") else get_param("install_root_remote_prod_use_time")
...
set_yaml <- function(p, form) {
  ...
  yaml <- "---"
  yaml <- c(yaml, readLines(fix_path(p$report_yaml_default)))
  if ("html" == form)       yaml <- c(yaml, readLines(fix_path(p$report_yaml_html))) else
  if ("pdf" == form)        yaml <- c(yaml, readLines(fix_path(p$report_yaml_pdf)))  else
  if ("word" == form)       yaml <- c(yaml, readLines(fix_path(p$report_yaml_word))) else
  if ("ioslides" == form)   yaml <- c(yaml, readLines(fix_path(p$report_yaml_ioslides))) else
  if ("powerpoint" == form) yaml <- c(yaml, readLines(fix_path(p$report_yaml_powerpoint))) else
    error_msg("Add yaml for ", form)
  
  if (p$report_table_of_content)    yaml <- c(yaml, readLines(fix_path(p$report_yaml_toc)))
...
create_report <- function(report) { # TODO rewrite code
  ...
  print_progress(report, "Creating report with title '", report$title, "' based on xlsx '", report$xlsx, "'(TODO)...")
  ...
  for (format in report$report_format) {
    ...
    if (is_set(report$report_text_file)) {
      add_default_text <- F
      report$report_text <- readLines(report$report_text_file)
    }
    
    # Create YML
    report <- set_yaml(report, format) # Only add yaml if not in report_text
    report$report_text[is.na(report$report_text)] <- "" # Replace NA's by empty lines
    # Save report text to report file
    print_progress(report, "Writing report text to file...")
    write(report$report_text, report$report_file) # write yaml (and user text)
    ...
    if (add_default_text) { # write default text if no custom text
      ...
      write(c("", readLines(fix_path(report$report_default_file))), report$report_file, append = T)      
    }

    # Do we want the appendix with all info from James?
    if (report$report_include_james_appendix) write(c("", readLines(fix_path(report$report_james_appendix_path))), report$report_file, append = T)
...
set_report_export <- function(p) {
  ...
  if (1 < length(index)) error_msg("You are creating a figure (", p$file_base, ".*) for your report. However, you have selected multiple output formats (*) for your figure. Please choose only one of jpg, pdf, png, svg.")
