...
geo_validate <- function(p) {
  ...
  if (is_set(p$geo_col_threshold)) if (is.unsorted(p$geo_col_threshold, strictly = T)) error_msg("The values in your parameter '", geo_col_threshold, "' should be strictly increasing.")
...
geo_pre <- function(p) {
  ...
  if (is.element("value", colnames(p$data))) {
    # cast value column
    p$data <- transform(p$data, value = as.numeric(value))
    ...
    for (i in 1:p$geo_n_regions) if (!is_set(p$region_color[i])) {
      ...
      for (j in 2:length(p$geo_col_threshold)) {
        if (!is.na(this_value)) if (p$geo_col_threshold[j - 1] <= this_value & this_value <= p$geo_col_threshold[j]) {
          ...
          rgb_col    <- colorRamp(p$palette[c(j - 1, j)], interpolate = "linear", space = "rgb")(this_value)
          hex_col    <- rgb(rgb_col[1,1], rgb_col[1,2], rgb_col[1,3], maxColorValue = 255)
...
geo_init_plot <- function(p) {
  ...
  # Margins
  mai <- c(p$margin_south, p$margin_west, p$margin_north, p$margin_east) / cm(1) # scale to inches
  ...
  # Init plot
  par(bg = p$bg_col, mai = mai, xaxs = 'i', yaxs = 'i')
...
geo_plot <- function(p) {
  ...
  if (is_set(p$region_label)) {
    for (i in 1:length(index_match)) {
      ...
      text(this_point[[1]], this_point[[2]], labels = set_newline(p$region_label[i]), family = p$font, col = get_col_from_p(p, p$region_label_col), font = p$region_label_font_style, cex = p$region_label_font_size)
    }
  }
