preprocess <- function(p) {
  ...
  # Inject font
  # if ("rijk" == p$font) quartzFonts(rijk = c("RijksoverheidSansText-Regular", "RijksoverheidSansText-Bold", "RijksoverheidSansText-Italic", "RijksoverheidSansText-BoldItalic"))
  ...
  for (function_name in preprocess_order) {
    ...
    if (exists(function_name) && 1 == length(formalArgs(function_name))) {
