format_labs_set_margin_west <- function(p) {
  ...
  p$x_lab <- stringr::str_replace_all(p$x_lab, "\\\\n", "\n")
  ...
  if (!is_set(p$y_lab)) {
    ...
    p$y_lab <- set_newline(p$y_lab) #p$y_lab <- stringr::str_replace_all(p$y_lab, "\\\\n", "\n")
  }
  
  # TODO y_lab right here?
  
  # set margin left based on label width
  y_lab_max_width          <- max(strwidth(if (p$turn) p$x_lab else p$y_lab, cex = if (p$turn) p$x_lab_font_size else p$y_lab_font_size, units = "inches", family = p$font)) * cm(1)
  
  if (creating_png_now()) { # Hack to make png look similar to pdf
    y_lab_max_width <- y_lab_max_width / 1.4
  }    
  ...
  if (is_set(p$group)) {
    if (p$turn) {
      group_lab_width      <- max(strwidth(unique(p$group), cex = p$group_font_size, units = "inches", font = hack_font(p, p$group_font), family = p$font)) * cm(1)
      if (creating_png_now()) { # Hack to make png look similar to pdf
        group_lab_width <- group_lab_width / 1.5
      }   
