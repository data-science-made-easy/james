user_line <- function(p) {
  ...
  if (is_set(val)) {
    abline(h = if (p$turn) val else NULL, v = if (p$turn) NULL else val, lwd = p$vline_dash_lwd, col = get_col_from_p(p, p$vline_dash_col), lty = p$vline_dash_lty)
  }  
  
  # TODO fix for dates
  if (is_set(p$hline_dash)) {
    abline(h = if (p$turn) NULL else p$hline_dash, v = if (p$turn) p$hline_dash else NULL, lwd = p$hline_dash_lwd, col = get_col_from_p(p, p$hline_dash_col), lty = p$hline_dash_lty)
  }
