...
box_pre <- function(p) {
  ...
  for (j in index_box) {
    new_quantiles     <- quantile(p$y[, j], p$box_quantiles, na.rm = TRUE)
...
jbox <- function(p) {
  ...
  for (j in seq_along(index_box)) {
    ...
    # First da box
    rect(co$xl, co$yl, co$xr, co$yh, col = this_col, border = NA)
    ...
    if (0 == p$box_median_shape) {
      lines(x = rep(p$quantile_matrix[j, 3], 2), y = c(co$yl - y_delta, co$yh + y_delta), col = median_col)
    } else {
      points(x = p$quantile_matrix[j, 3], y = mean(c(co$yl, co$yh)), col = median_col, pch = p$box_median_shape, cex = p$box_median_shape_size)
    }
    ...
    for (w in c(0, 3)) {
      ...
      lines(x = c(co$xl, co$xr), y = c(co$yl, co$yh), col = this_col, lwd = p$line_lwd)
    }
    ...
    if (p$box_median_lab_show) {
      ...
      median_lab_height <- strheight(median_lab, cex = p$box_median_lab_font_size)

      if (0 == p$box_median_shape) { # adapt to line
        text(x = p$quantile_matrix[j, 3], y = y_box_median_lab_below - median_lab_height, labels = median_lab, pos = 4, cex = p$box_median_lab_font_size, offset = 0.15)
      } else { # adapt to symbol
        text(x = p$quantile_matrix[j, 3], y = y_box_median_lab_middle + 1.5 * median_lab_height, labels = median_lab, cex = p$box_median_lab_font_size, offset = 0)
      }
    }
  }
