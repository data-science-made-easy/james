...
scaling_factor <- function(p) {
  ...
  for (i in 1:n_cols) txt_width_per_col[i] <- max(strwidth(name[(i-1) * p$legend_n_per_column + 1:p$legend_n_per_column], font = hack_font(p, p$legend_font_style), adj = 0, cex = p$legend_font_size, family = p$font), na.rm = T)
...
j_legend <- function(p) {
  ...
  for (j in this_order) {
    ...
    if (is.element(p$legend_type[j], LINE_SET)) {
      lines(x_left + c(0, symbol_width), rep(y_line, 2), col = get_col_from_p(p, p$legend_color[j]), lwd = p$line_lwd[j], lty = p$line_lty[j])
      ...
      if (0 != p$line_symbol[j]) {
        ...
        points(x_left + symbol_width / 2, y_line, pch = p$line_symbol[j], col = this_col, cex = p$line_symbol_size[j])
      }
    }
      
    # Legend block
    if (is.element(p$legend_type[j], LEGEND_BLOCK)) polygon(x = c(x_left, x_left + symbol_width, x_left + symbol_width, x_left), y = c(y_line, y_line, y_line + symbol_height, y_line + symbol_height) - symbol_height / 2, col = get_col_from_p(p, p$legend_color[j]), border = NA)
    ...
    if (BOX == p$legend_type[j]) {
      ...
      if (p$turn) {
        ...
        lines(x_left + c(0, whisker_length), rep(y_line, 2), col = this_col, lwd = p$line_lwd)
        lines(x_left + symbol_width - c(0, whisker_length), rep(y_line, 2), col = this_col, lwd = p$line_lwd)
        ...
        polygon(x = x, y = y, col = this_col, border = NA)
      } else {
        box_height     <- symbol_height / 2
        whisker_length <- (symbol_height - box_height) / 2
        lines(rep(x_left + symbol_width / 2, 2), c(y_line, y_line + whisker_length) - symbol_height / 2, col = this_col, lwd = p$line_lwd)
        lines(rep(x_left + symbol_width / 2, 2), c(y_line + whisker_length + box_height, y_line + symbol_height) - symbol_height / 2, col = this_col, lwd = p$line_lwd)
    ...
    if (is.element(p$legend_type[j], LEGEND_STACK)) {
      ...
      for (j_stack in seq_along(p$legend_stack_vec)) {
        ...
        rect(x_left, y0, x_left + symbol_width, y0 <- y0 + piece_height, col = this_col, border = NA)
      }
    }
    ...
    if (is_dot(p$legend_type[j])) {
      ...
      points(x_left + symbol_width / 2, y_line, col = this_col, pch = this_pch, lwd = if (1 == length(p$dot_lwd)) p$dot_lwd else p$dot_lwd[is_nth_dot(j, p)]) # , cex = p$dot_size ... or  keep constant cex constant?
    }
    
    if (LEGEND_WHISKER == p$legend_type[j]) {
      whisk_line <- function(x, y) lines(x, y, col = get_col_from_p(p, p$legend_color[j]), lwd = p$whisker_lwd)
      if (p$turn) {
    ...
    #     polygon(x = c(x_left, x_left + symbol_width, x_left + symbol_width, x_left), y = c(y_line, y_line, y_line + symbol_height, y_line + symbol_height) - symbol_height / 2, col = p$forecast_bg_col, border = NA)
    #   }
    #   # polygon(x = c(x_left, x_left + symbol_width, x_left + symbol_width, x_left), y = c(y_line, y_line, y_line + symbol_height, y_line + symbol_height) - symbol_height / c(4, 4, 2, 2), col = "black", border = NA, density = cm(1) * p$forecast_shading_density, angle = p$forecast_shading_angle)
    #   #
    ...
    # Legend text
    text(x_left + symbol_width + p$legend_space_symbol_txt / p$width, y_line, labels = p$name[j], font = hack_font(p, p$legend_font_style), adj = 0, cex = scaling * p$legend_font_size, family = p$font)
    
    legend_txt_length <- max(legend_txt_length, strwidth(p$name[j], font = hack_font(p, p$legend_font_style), cex = p$legend_font_size, family = p$font))
  }
