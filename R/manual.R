...
isolate_xlsx_and_r_script <- function(p) {
  ...
  # If data is not from a data tab, then use 'inner data'
  use_inner_data <- !is_set(p$tab) & is_set(p$data)
  if (use_inner_data) {
    meta_table <- rbind(c("tab", "data"), meta_table)
  } else if (is_set(p$tab)) {
    meta_table <- rbind(c("tab", p$tab), meta_table)
  }
  ...
  if (is_set(p$tab)) {
    ...
    openxlsx::addWorksheet(wb, p$tab)
    openxlsx::writeData(wb, p$tab, x = manual_data_tab, xy = c("A", 1))
  } else {
    if (use_inner_data) {
  ...
  # (4) return paths
  list(xlsx = fix_path(fix_path_if_manual_local_dev(result_file_xlsx)), rdata = fix_path(fix_path_if_manual_local_dev(result_file_r_data)), r = fix_path(fix_path_if_manual_local_dev(result_file_r)))
}
...
figure_with_params <- function(report, id = "", ...) {
  ...
  # Set file name
  if (!is_set(p$file_base) & is_set(p$id)) p$file_base <- p$id
  ...
  for (i in seq_along(param_name)) {
    if (1 == length(param_value[[i]])) if (stringr::str_detect(param_value[[i]], "\\\\n")) {
      param_value[[i]] <- stringr::str_replace(string = param_value[[i]], pattern = "\\\\n", replacement = "\\\\\\\\\\\\\\n") # amazing, this number of \, isn't it? :-O
    }
  }
  # If data is not from a data tab, then use 'inner data'
  use_inner_data <- !is_set(p$tab) & is_set(p$data)
  if (use_inner_data) {
    param_name <- c(param_name, "tab")
    param_value[["tab"]] <- "data"
  }  
  # Fix params that have multiple values
  # for (i in seq_along(param_value)) param_value[i] <- paste(param_value[[param_name[i]]], collapse = ", ")
  ...
  # path <- plot(p)
  # rmd_link_to_figure <- if ("pdf" == p$report_output_format) paste0("\\includegraphics{", path, "}") else paste0("![](", path, ")")
  #
  # pos_bracket <- tail(stringr::str_locate(string = rmd_link_to_figure, pattern = "]\\("), 1)[1, 2]
  # p$figure_path <- stringr::str_sub(stringr::str_sub(rmd_link_to_figure, end = -2), start = 1 + pos_bracket)
  ...
  html <- paste0(c(html,
    "<span style='color:#43B546'><B>How to reproduce this figure:</B></span> copy [this xlsx-file](", path$xlsx, ") and [this batch file](", file.path(get_param("install_root_remote_prod_use_time"), get_param("james_version"), paste0("james-", get_param("james_version"), ".bat")), ") to a personal directory and start the batch file. You can also run [this R-script](", path$r, ") in R.",
    "</td></tr>",
  "</table>"), collapse = "\n")
...
describe_params_with_help_appendix_list <- function(report) {
  all_params <- sort(unique(globals$param[which(!is_no(globals$in_manual) & is_set(globals$help))]))
...
describe_param <- function(report, param, add_bold_name = TRUE) {
  # Describe the param
  index <- which(param == globals$param)
  p     <- globals[index, ]
  ...
  if (is_set(p[["list-type"]])) {
    ...
    txt <- paste0(txt, " The value is a list with elements of the type '", this_type, "' separated by '", p$type, "'.")
  } else {
    this_type <- p$type
    txt <- paste0(txt, " The value is of type '", this_type, "'.")
  }
  ...
  available_styles <- colnames(globals)
...
describe_param_set <- function(report, param_set) {
  ...
  this_help <- globals$help[index[1] - 1]
  this_help <- paste0(this_help, see_also(globals$see_also[index[1] - 1]))
  txt <- if (!is_set(this_help) | "NA" == this_help) "" else paste(this_help, " \n\n")
  
  # Sort parameters on alphabet
  index <- index[sort(globals$param[index], index.return = T)$ix]
  
  for (i in index) {
    if (!is_no(globals$in_manual[i])) txt <- paste(txt, "*", describe_param(report, globals$param[i]), "\n")
  }
...
get_param_settings_not_in_any_R_file <- function() {
  p_not_found <- NULL
  params <- globals$param[which("#" != stringr::str_sub(string = globals$param, end = 1))]
  all_files <- NULL
  for (subdir in c("R", "ext/snippet", "ext/script")) {
...
get_style_parameters <- function(style) {
  ...
  index <- which(!is.na(globals[, name]))
  val   <- globals[index, name]
  
  var  <- globals[index, 1]
  unit <- globals[index, UNIT]
