...
update_y_lims <- function(p, j, vec) {
  ...
  if (all(is_yl(p, j))) {
    ...
    p$y_right_lim <- range(as.numeric(c(p$y_right_lim, vec)), na.rm = T)
...
secondary_y_axis <- function(p) {
  print_debug_info(p)
  if (all("l" == p$y_axis) | is_set(p$y_right_lab)) return(p)

  # print_info(p, "left y-axis: ", paste(p$y_lim, collapse = ", "))
  # print_info(p, "right y-axis: ", paste(p$y_right_lim, collapse = ", "))
  ...
  # PREVENT ERRORS
  scaling_data <- max(p$y_lim / p$y_right_lim, na.rm = T)
  ...
  while (!done & 0 < tries_left) {
    ...
    while (!done & i <= length(m_possible)) { # try different number of labs on right axis
      labs <- pretty(scaling * c(0, p$y_right_lim[2]), n = m_possible[i])
  ...
  if (!done) error_msg("Sorry :-O James found no suitable scaling for the secondary y-axis. Please try so manually with parameters y_right_scaling, y_right_at and y_right_lab.")

  # Do da scaling
  p$y[, which(is_yr(p))] <- p$y_lim[2] / tail(labs, 1) * p$y[, which(is_yr(p))]
  
  # Set da labs
  p$y_right_lab <- rep(NA, y_left_n_labels)
  p$y_right_lab[which(seq_left %in% seq(0, 1, length = length(labs)))] <- labs

  # fix y_lab right elsewhere
  if (is_set(p$y_right_lab)) {
    p$y_right_lab <- fix_numbers(p$y_right_lab, p$y_n_decimals, p$decimal_mark, big_mark = if (p$y_lab_big_mark_show) p$big_mark else "")
  } else {
    p$y_right_lab <- set_newline(p$y_right_lab)
  }
